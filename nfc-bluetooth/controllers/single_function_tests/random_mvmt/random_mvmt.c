...
int main() {
  ...
  int rand_num = (rand() % 2000);// + (10 * atoi(wb_robot_get_name() + 5));
  int i = 0;
  ...
  while (wb_robot_step(TIME_STEP) != -1) {
    ...
    // if (z < 0) {
      // bear = 270 - (atan(x / -z) * 180 / M_PI);
      // bear = bear;
    // } else {
      // bear = (atan(x / z) * 180 / M_PI);
      // bear = bear;
    // }
    ...
    if (i>rand_num) {
      ...
      if (i > (rand_num * 1.2)) {
        i = 0;
        rand_num = (rand() % 2000);// + (rand() % 10 * atoi(wb_robot_get_name() + 5) + 1);
      }
      i += 1;
