...
int oam_active, oam_reset, oam_obst, obst_direction, oam_obst_block = FALSE, obst_count = 0;
int oam_speed[2];
...
int num = 0, speed[2], counter = 0, rand_num, my_ID;
float turn;
FILE *fpt;

void MovementModule(void) {
  
  if (oam_active == TRUE && message_sent == FALSE) {
...
void TurnToObstacle(void) {

  for (int i=0; i<100; i++) { // TO DO need to test the connection
...
int main() {
  int ps_offset[NB_DIST_SENS] = {0, 0, 0, 0, 0, 0, 0, 0}, i;
  ...
  wb_emitter_set_range(emitter, 0.05);
  ...
  while(wb_robot_step(TIME_STEP) != -1 && wb_robot_get_time() < (10800 * 5)) {  // Main loop for max 3 hours
    ...
    if (oam_obst == TRUE && oam_obst_block == FALSE) {
      TurnToObstacle();
    }
    else {
      MovementModule();
